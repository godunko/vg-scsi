...
package body A0B.SCSI.Decoders.SPC.INQUIRY is
   ...
   is
      ...
      use type A0B.SCSI.SPC5.VPD_Page_Code;
      use type A0B.Types.Reserved_1;

   begin
      if not Self.Check_USB_MSC_BOOT_CDB_Length
        (A0B.SCSI.SPC5.CDB.INQUIRY_CDB_Length, CDB_Storage)
      then
         return False;
      end if;

      declare
         CDB : constant A0B.SCSI.SPC5.CDB.INQUIRY_CDB
           with Import, Address => CDB_Storage'Address;

      begin
         pragma Assert (CDB.OPERATION_CODE = A0B.SCSI.SPC5.INQUIRY);
