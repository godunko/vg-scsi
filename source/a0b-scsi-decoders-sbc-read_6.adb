...
package body A0B.SCSI.Decoders.SBC.READ_6 is
   ...
   begin
      if not Self.Check_CDB_Length
        (A0B.SCSI.SBC3.CDB.READ_6_CDB_Length, CDB_Storage)
      then
         return False;
      end if;

      declare
         CDB : A0B.SCSI.SBC3.CDB.READ_6_CDB
           with Import, Address => CDB_Storage'Address;

      begin
         pragma Assert (CDB.OPERATION_CODE = A0B.SCSI.SBC3.READ_6);
