...
package body A0B.SCSI.Decoders.SBC.WRITE_12 is
   ...
   begin
      if not Self.Check_CDB_Length
        (A0B.SCSI.SBC4.CDB.WRITE_12_CDB_Length, CDB_Storage)
      then
         return False;
      end if;

      declare
         CDB : constant A0B.SCSI.SBC4.CDB.WRITE_12_CDB
           with Import, Address => CDB_Storage'Address;

      begin
         pragma Assert (CDB.OPERATION_CODE = A0B.SCSI.SBC4.WRITE_12);
         ...
         Descriptor :=
           (Variant               => A0B.SCSI.Commands.SBC.WRITE_12,
